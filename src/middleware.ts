...
if (typeof window === 'undefined') {
  import('@/lib/server-init').catch(logger.error)
}

export function middleware(request: NextRequest) {
  logger.log('Middleware: Проверка маршрута:', request.nextUrl.pathname)
  ...
  if (request.nextUrl.pathname.startsWith('/courier') && 
    ...
    logger.log('Middleware: Токен найден:', !!token, token ? `Длина: ${token.length}` : 'Токен отсутствует')

    if (!token) {
      logger.log('Middleware: Токен отсутствует, перенаправление на логин')
    ...
    if (!token || token.length < 10) {
      logger.error('Middleware: Недействительный токен, перенаправление на логин')
    ...
    logger.log('Middleware: Токен действителен, разрешаем доступ')
  }
